...
现有 `pastis_ext_prior_draft.csv` 中的手工小数仅用于接口联调和检查月份索引，不能作为正式实验的外部先验，也不能在论文或汇报中解释为专家权重。正式版本必须由原始资料表和固定规则生成。

### 4.1 原始记录

每条农学记录至少包含：`class_id`、`region`、`phase`、`start_period`、`end_period`、`source`、`year`、`crop_use_or_variety` 和 `confidence`。生育阶段统一为播种、出苗/返青、快速生长、开花、成熟和收获；原始资料没有提供的信息记为缺失，不能凭印象补数值。
...
```text
E[class, region, calendar_month, phase] in {0, 1, missing}
```

`1` 表示来源明确支持该地区在该月处于相应阶段，`0` 表示来源明确排除，`missing` 表示资料不足。没有资料不能自动改写为 `0`。

### 4.3 二维矩阵生成

为兼容当前 `P_ext[class, month]` 接口，先在实验方案中预先定义需要纳入的目标阶段集合，再按有效地区的支持比例聚合：

```text
P_ext(c,m) = count(valid regions supporting a target phase for class c at month m)
             / count(valid regions for class c at month m)
```

分母只统计具有有效资料的地区。若某类别在该月份没有任何有效地区记录，则保持中性并写入缺失标记；不得用任意小数代替。若一个地区存在多份来源，应先在地区内合并，避免资料较多的地区被重复加权。

### 4.4 不确定类别

- `direct` 类别按四个 PASTIS 区域或最接近的可用地区资料聚合；
- `ambiguous` 类别只有在用途或亚型得到确认后才生成正式曲线，否则扩大不确定性或保持中性；
- `group` 类别不套用单一代表作物，优先保持中性或单独使用 `P_data`；
- Background 使用中性先验，Void 不进入矩阵。

所有来源、阶段映射、目标阶段集合、地区内合并方式和缺失值策略必须在训练前冻结并写入 manifest。这样生成的数值表示“现有来源在不同地区的一致支持程度”，不是类别概率或真实物候标签。
...
物候旁路不再写进 `3D-Aware DPT` 的内部。它现在在**冻结 Galileo 特征与 decoder 之间**工作：对每个选中的时间保留层，把 `P[class, month]` 投影为与 Galileo hidden state 对齐的 `768` 维残差，再按空间广播相加：

```text
F[l,t]:              [B, T, 768, H, W]
q_t = Adapter(P):    [B, T, 768, 1, 1]
F'[l,t] = F[l,t]+q:  [B, T, 768, H, W]
                       -> 任意保留 T 的 decoder
```

因此，`3D-Aware DPT`、temporal single-layer/multi-layer DPT、temporal UPerNet 和 temporal Galileo-DPT 可以读取同一份增强特征；它们只在随后如何做时间、层级和空间融合上不同。已经平均掉 `T` 的旧 2D cache 仍不能使用这个逐月旁路。

当前第一阶段仍只训练 3D-Aware DPT，是为了先回答一个收敛的问题，而不是因为旁路只能用于它。后续工作是多随机种子和 test-only 最终评估流程。
...
当前旁路由 `CachedFeatureSegmentation` 在调用 decoder 前执行：先验由 `P[class, month]` 经 `PhenologyPriorAdapter` 投影到 Galileo 的 `768` 个特征通道，并在每层、每个时间步以残差形式加入缓存特征。Galileo encoder 保持冻结，时间轴保持完整；3D-Aware DPT 随后才将特征投影到自己的 `256` 个内部通道。
