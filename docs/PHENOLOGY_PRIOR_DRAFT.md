...
- `alpha=0`、uniform、class-shuffled、month-shifted 的自动对照实验；
- 多随机种子和 test-only 最终评估流程。

因此组员可以马上运行 `3D-Aware DPT` 的无先验/外部先验对照，但在完成上述受控实验前，不应报告“物候先验已经普遍提升了所有 decoder”。
...
- 无先验：`configs/galileo_3d_aware_dpt.yaml`，`phenology.enabled: false`；
- 外部先验：`configs/galileo_3d_aware_dpt_phenology_ext.yaml`，`phenology.enabled: true`；
- 数据先验：复制外部先验配置，只将 `phenology.path` 改为 `data/priors/pastis_data_prior_draft.csv`。
