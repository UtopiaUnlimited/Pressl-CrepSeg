...
不得从“感觉上更重要”直接填写 12 个权重。正式流程先将来源转换为结构化阶段张量：

```text
E[class, region, calendar_month, phase] in {0, 1, missing}
```

其中 `phase` 至少区分播种、出苗/返青、快速生长、开花、成熟和收获。随后预先冻结目标阶段集合，并按具有有效资料的地区覆盖比例生成当前模型所需的二维矩阵：

```text
P_ext(c,m) = 支持类别 c 在月份 m 处于目标阶段的有效地区数
             / 类别 c 在月份 m 具有有效资料的地区总数
```

同一地区存在多份来源时先在地区内合并，避免按网页数量重复加权。没有资料记为缺失，不能解释为不生长，也不能以任意小数补齐。聚合类别、用途不明类别和 Background 按 [`PHENOLOGY_PRIOR_SOURCES.md`](PHENOLOGY_PRIOR_SOURCES.md) 中的映射等级采用中性或不确定处理。生成脚本必须同时输出来源清单、映射规则和 manifest，使每个正式权重能够追溯到原始记录和确定性计算过程。

当前 `pastis_ext_prior_draft.csv` 的手工软值只允许用于代码接口联调；在上述资料表和生成规则冻结前，不得用于正式效果结论。
...
第一阶段固定使用保留完整 `T=12` 的 `3D-Aware DPT`，只为先验证先验本身是否有效。公共先验接口同时兼容 temporal-readout 的单层、多层、Adapted DPT 与 UPerNet，但这些组合留到第二阶段，不能与第一阶段的先验因果检验混在一起。P0/P1/P2 三次运行除 overlay 和日志目录外完全相同：相同 `temporal_v2` 缓存、frozen Galileo、模型、loss、Prodigy、AMP、有效 batch、early stopping 和 `best_val_miou` 选择规则。

| 编号 | 配置 | 先验输入 | 回答的问题 |
| --- | --- | --- | --- |
| P0 | `galileo_3d_aware_dpt.yaml` | 无 overlay；`PhenologyPriorAdapter` 不构造 | 当前 3D-Aware DPT 晚期融合的严格基线 |
| P1 | 基础配置 + `phenology/external.yaml` | 正确的 `P_ext` | 人工物候先验是否提升模型 |
| P2 | 基础配置 + `phenology/external_class_shuffled.yaml` | 固定类别置乱的 `P_ext` | 提升是否来自正确类别-月份对应，而非额外分支参数 |
