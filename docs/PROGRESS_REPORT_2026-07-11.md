# 阶段成果报告：Galileo 冻结特征上的 Decoder Baseline 实验
...
本阶段完成了 Galileo 论文输入协议对齐、共享特征缓存、最终层卷积 baseline、多层同尺度融合 baseline 训练，以及 fold5 test 集完整评估。

命名校正：代码和配置中的 `single_layer_dpt`、`multi_layer_dpt` 是历史名称。前者没有使用多层特征，后者虽然融合多层特征，但所有层都保持 `16×16`，均未实现 DPT 原论文的多尺度 reassemble 与渐进融合。因此本文将它们作为 DPT 前置 baseline，而不作为完整 DPT 结果。多尺度 Galileo-DPT 正在实现，数据与结论待补。
...
| 最终层卷积 baseline | final `features` | 1.03058 | **45.78%** | **+6.58 pp** |
| 多层同尺度融合 baseline | layers 3/6/9/12，均为16×16 | **1.00184** | **46.32%** | **+7.12 pp** |
| 多尺度 Galileo-DPT | layers 3/6/9/12，多尺度重组 | 待补 | 待补 | 待补 |

在当前实验设置下，多层同尺度融合 baseline 比最终层卷积 baseline 提高 **0.54 个百分点 mIoU**，同时 test loss 降低 0.02874。结果支持“Galileo 中间层信息可能对 PASTIS 分割有帮助”，但增益目前较小，且不同类别上的变化并不一致。该结论不能外推为“DPT 有效”。

需要强调：论文中的 39.2% 是 Galileo ViT-Base 在 Table 17 中的 **linear probing** 结果；本项目两个 baseline 具有额外的卷积细化、融合和上采样能力。因此超过 39.2% 可以说明更强 decoder 在当前协议下有效，但不能解释为同容量、同训练策略下对论文线性 probe 的严格复现或公平替代。
...
本文后续使用“论文参考基线”指代该 39.2%；使用“最终层卷积 baseline”指代 45.78% 的本项目最小空间 decoder。

## 3. 数据与输入协议

本轮两个 decoder baseline 严格共享以下设置：
...
### 4.1 最终层卷积 Baseline（历史名：Single-Layer DPT）
...
### 4.2 多层同尺度融合 Baseline（历史名：Multi-Layer DPT）
...
可训练参数：9,058,067（约 9.06M），约为最终层卷积 baseline 的 1.84 倍。

### 4.3 多尺度 Galileo-DPT（正在实现）

目标结构为 layer 3/6/9/12 独立投影后重组为 `64×64、32×32、16×16、8×8` 四级特征，再由深到浅渐进融合。该结构继续复用现有缓存和冻结 encoder。以下内容待实现与训练完成后填写：
...
| --- | ---: | ---: |
| 最终层卷积 baseline | 697.8MiB | 3.24h |
| 多层同尺度融合 baseline | 880.0MiB | 3.94h |
...
| --- | ---: | --- | ---: |
| 最终层卷积 baseline | 25 | 最低 val loss | 47.07% |
| 多层同尺度融合 baseline | 11 | 最低 val loss | 47.95% |
...
| --- | ---: | ---: | ---: | ---: | ---: |
| 最终层卷积 baseline | epoch 25 | 1.03058 | 45.78% | - | +6.58 pp |
| 多层同尺度融合 baseline | epoch 11 | **1.00184** | **46.32%** | **+0.54 pp** | **+7.12 pp** |
| 多尺度 Galileo-DPT | 待补 | 待补 | 待补 | 待补 | 待补 |
...
2. **最终层卷积 decoder 已成为有效 baseline。** 其 test mIoU 为 45.78%，在当前协议下明显高于论文线性探测参考值 39.2%。
3. **同尺度多层特征存在初步正收益。** 多层同尺度融合 baseline 达到 46.32%，比最终层 baseline 高 0.54 个百分点，但参数量增加约 84%，训练时间增加约 22%。
4. **完整 DPT 尚不能评价。** 多尺度 Galileo-DPT 正在实现，当前 46.32% 不能作为其结果或上限。
5. **100 epochs 明显过长。** 两个模型后半程都出现 train loss 下降、val loss 上升和 val mIoU 回落，尤其多层模型更严重。
6. **保存策略与论文主指标错位。** 当前按 val loss 保存，而主要汇报指标是 mIoU；多层模型因此没有保留 val mIoU 最高的 epoch 22。

## 8. 局限性

- 当前每个 decoder 只运行了 seed 42，无法给出均值、标准差或显著性判断。
- 39.2% 来自论文 linear probing；本项目两个卷积 baseline 参数更多，比较反映的是完整下游方案差异，不是纯 encoder 表征能力差异。
- 论文还包含 normalization、学习率搜索和多次运行；本轮使用固定的公开候选配置，没有复现完整 sweep。
- 当前只保存最低 val loss checkpoint，无法补测历史最高 val mIoU checkpoint。
- 多层同尺度 baseline 参数量约为最终层 baseline 的 1.84 倍，当前结果还没有控制 decoder 参数量。
- 多尺度 Galileo-DPT 尚无正式实现结果，不能预先写入性能结论。
...
1. 完成多尺度 Galileo-DPT，并验证四级 reassemble shape、渐进融合和输出尺寸。
2. 修改 Trainer，同时保存 `best_val_loss.pt` 与 `best_val_miou.pt`，加入以 val mIoU 为依据的 early stopping。
3. 在完全相同的缓存、loss 和 fold 下训练多尺度 DPT，补充 val/test、参数量、显存和定性结果。
4. 将最大 epoch 降到约 40，并设置 patience 10～15；具体规则应在下一轮运行前固定。
5. 训练已经实现的 UPerNet-style decoder，并保持完全相同的缓存、loss、batch size 和 seed 列表。
6. 对关键模型使用至少 3 个相同 seeds，并增加参数量匹配、混淆矩阵和类别频率分析。

## 10. 复现命令

最终层卷积 baseline test（历史配置名）：
