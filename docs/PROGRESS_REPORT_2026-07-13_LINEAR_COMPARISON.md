...
### 2.3 追加记录：3D-Aware DPT

2026-07-15 收到组内提供的一组 3D-Aware DPT fold5 测试输出：

| 方法 | Test loss | Test mIoU |
| --- | ---: | ---: |
| 3D-Aware DPT | 1.03279 | **59.945%** |

原始逐类 IoU 为：

```text
0.73111, 0.61606, 0.82834, 0.85413, 0.73276, 0.88384, 0.56744, 0.68865, 0.49959, 0.85782, 0.36777, 0.59229, 0.36341, 0.51119, 0.36512, 0.81174, 0.40846, 0.39818, 0.31166
```

该结果暂作为“已收到的阶段性结果”，因为配置、commit、seed、checkpoint、最佳验证轮次和 temporal cache 目录尚未同步到本报告。补齐这些元数据并确认与前述输入协议一致后，才能将其纳入正式跨 decoder 对比。
...
4. 补齐 3D-Aware DPT 的配置与 checkpoint 审计信息，并训练 Galileo-Adapted 2D DPT、UPerNet-style，保持同一 fold、输入和 checkpoint 规则。
