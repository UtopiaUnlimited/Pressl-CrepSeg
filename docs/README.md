...
1. [当前唯一执行规划：V4 类别-时序原型记忆](NEXT_STAGE_V4_CLASS_TEMPORAL_PROTOTYPE_PLAN_2026-07-21.md)
...
| 文档 | 作用 | 使用规则 |
| --- | --- | --- |
| [NEXT_STAGE_V4_CLASS_TEMPORAL_PROTOTYPE_PLAN_2026-07-21.md](NEXT_STAGE_V4_CLASS_TEMPORAL_PROTOTYPE_PLAN_2026-07-21.md) | **唯一总规划**；定义 V3 的停止边界、V4 原型记忆、对照与实验顺序 | 发生冲突时以它为准 |
...
| [M2_M3_CLIMATE_SOIL_PRIOR_CATALOG.md](M2_M3_CLIMATE_SOIL_PRIOR_CATALOG.md) | M2 气象与 M3 土壤先验的资料、字段、接入和泄漏边界 | V3 的资料审计；不再作为下一轮数据源 |
| [M4_GEOGRAPHIC_PRIOR_AND_SOURCE_BALANCING.md](M4_GEOGRAPHIC_PRIOR_AND_SOURCE_BALANCING.md) | M4 地理上下文、通用静态数值 adapter 与多源注意力平衡 | V3 的资料边界与诊断解释；接口保留但不再作为主输入 |
| [NEXT_STAGE_HETEROGENEOUS_PRIOR_INJECTION_PLAN_2026-07-17.md](NEXT_STAGE_HETEROGENEOUS_PRIOR_INJECTION_PLAN_2026-07-17.md) | V2 CA-HPI 与 V3 SA-SFiLM 的结构、接口和历史实验设计 | 作为已实现方法的技术记录，不生成新任务 |
...
2. **方法：** V2 CA-HPI 与 V3 SA-SFiLM 保留为对照；当前创新是 V4 类别-时序原型记忆。
