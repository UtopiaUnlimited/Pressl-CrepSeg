...
- 数据输入的维度问题
    ...
    - 前四种
        - 旧实验沿用官方 probing 接口，在 encoder 输出端对上下文化后的逐月特征求均值；这保留了一部分时序上下文，但 decoder 无法再显式区分月份
        - 新实验保留相同二维空间 decoder，在其入口使用月份感知时间读出，逐层、逐空间位置学习 T 权重
    - 第五种
        - 正是保留 T 维并在 decoder 中继续建模时间，用来检验这种压缩是否限制了分割性能
    - 已添加的时间读出消融
        - Galileo 仍同时处理12个月；decoder 不重复完整时间 Transformer，只学习如何读取已经上下文化的逐月输出
    - 后继续添加二
    ...
    - 关于特征缓存
        ...
        - 旧方案一至四读取新缓存时仍可临时沿 T 求均值，作为固定读出对照
        - 新方案一至四统一经过可学习月份读出，方案五直接使用完整 T 到 3D decoder 后端
    - 关于 decoder 空间重采样
