...
## 7. DPT 论文与当前项目实现的准确对应

这里需要严格区分论文原始 DPT、当前已经完成的两个 baseline，以及正在实现的多尺度 Galileo-DPT。代码中的旧名称不等于论文方法已经复现。

### 7.1 外部参考：Galileo 论文 linear probe

Galileo 论文在 PASTIS 上报告的是 probing 参考结果：

```text
frozen Galileo encoder + linear probing head
```

它主要回答冻结表征是否具有线性可分性，不是与当前卷积 decoder 同容量的公平消融。因此 39.2% 应称为论文外部参考，而不是本项目内部 baseline。

### 7.2 Baseline 1：最终层卷积 decoder

代码中的 `single_layer_dpt` 只读取 Galileo 最终层特征：
...
它使用已经恢复好的二维空间特征，再进行卷积细化和一次性上采样。它没有多层 token、没有多尺度 reassemble，也没有渐进融合。

因此更准确的称呼是：

```text
final-layer convolutional decoder
```

它是 DPT 之前的空间 decoder baseline，而不是 DPT 复现。

### 7.3 Baseline 2：多层同尺度融合 decoder

代码中的 `multi_layer_dpt` 读取 Galileo 的第 3、6、9、12 层，但四层恢复后都是 `16×16`：

```text
Galileo layers 3/6/9/12
    -> each [B, 768, 16, 16]
    -> independent 1×1 projection
    -> deep-to-shallow same-resolution addition
    -> one-time upsampling
    -> segmentation map
```
...
但它只验证“多层特征是否有用”，没有验证“多尺度 DPT 是否有效”。当前 46.32% 来自这个 baseline，不能称为 DPT 的实验结果。

### 7.4 主方法：多尺度 Galileo-DPT（正在实现）

正在实现的结构才正式适配 DPT 的核心流程：

```text
Galileo layers 3/6/9/12，each 16×16
...
它保留 DPT 的两个关键点：多层 Transformer 特征和多分辨率渐进融合。由于 Galileo 的 token 结构不同于普通图像 ViT，本项目使用已经验证的结构化空间聚合，不机械复制 class-token readout。因此应称为“适配 Galileo 的多尺度 DPT”，而不是逐行复现原始代码。

当前状态：实现中。参数量、val/test mIoU、逐类结果和结论均待训练后填写，不能预设一定优于 baseline。

### 7.5 替代路线：UPerNet-style decoder
...
多层同尺度融合 decoder
        ↓ 检验跨 Transformer 层信息
多尺度 Galileo-DPT
        ↓ 检验 reassemble 与渐进融合，正在实现
UPerNet-style decoder
        ↓ 比较另一种融合机制
```

其中前两个已经完成的结构都是控制实验。只有多尺度 Galileo-DPT 同时包含多层读取、多尺度重组和渐进融合，才对应 DPT 原论文的核心思想。

当前最终层卷积 baseline 高于论文 linear probe，说明更强空间读取具有初步价值；同尺度多层融合再提高 0.54 个百分点，只能说明跨层信息可能有用。多尺度 DPT 的效果仍需等待正式结果。如果完整 DPT 没有超过两个 baseline，这同样是有意义的结果，可能说明 Galileo 不同深度并不天然对应图像 ViT 的尺度层级，或者当前时序聚合与冻结 encoder 才是主要瓶颈。
...
对我们的项目来说，它既解释了为什么 DPT 值得验证，也明确提醒我们：仅仅读取多层特征并相加还不是 DPT。只有完成多尺度 reassemble 与渐进融合后，才能评价 DPT 对 Galileo 作物分割是否有效。
